...
TEST(ALGOfilter, Handle) {
  ...
  std::vector<AlgoFilter::FilterTarget> filter_set_target;
  AlgoFilter::Input input;
  input.radar_set_state = &radar_set_state;
  AlgoFilter::Output output;
  output.targets = &filter_set_target;

  AlgoFilter algo_filter;
  algo_filter.Handle((void*)&input, (void*)&output);
}
