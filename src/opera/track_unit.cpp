...
void TrackUnit::Execute() {
  ...
  while (distance_current + distance_dot < distance_sum ||
      DoubleEqual(distance_current + distance_dot, distance_sum)) {
    ++tick_current;
  ...
  if (!DoubleEqual(distance_current + distance_dot, distance_sum)) {
    ++tick_current;
    //TODO: something todo
...
void TrackUnit::Iterator::Next() {
  ...
  speed_current_ += interval_
      * track_unit_->acc_->GetAcceleration(tick_current_ * interval_);
...
void TrackUnit::Iterator::Value(TrackUnitState& track_unit_state) const {
  ...
  track_unit_state.acc = track_unit_->acc_->GetAcceleration(tick_current_
      * interval_);
